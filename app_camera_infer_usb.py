...
def render_resultado_usb(res: Optional[dict]) -> None:
    ...
    html = f"""
    <div style="border-radius:12px;padding:16px;background:{ui['bg']};border:3px solid {ui['border']};text-align:center;margin-top:10px;">
...
def send_report_email(pdf_path: str | Path, html_path: str | Path | None = None) -> tuple[bool, str]:
    ...
    smtp_user = str(st.session_state.get("smtp_user", "")).strip()
    smtp_password = str(st.session_state.get("smtp_password", "")).strip()
    sender_name = str(st.session_state.get("email_sender_name", "SVC USB")).strip() or "SVC Inspeção de Molas"
...
with colB:
    try:
        ...
        if is_eng:
            ...
            use_roi_crop = st.checkbox(
                "Usar ROI USB ao salvar",
                key="usb_use_roi_crop_checkbox",
                value=bool(st.session_state.get("usb_use_roi_crop", False)),
            )
            st.session_state["usb_use_roi_crop"] = bool(use_roi_crop)

            top1, top2 = st.columns([2.2, 1.0])
            with top1:
                if st.button("✅ Confirmar OK", key="usb_btn_ok", use_container_width=True):
                    try:
                        saved = save_usb_manual_sample("OK", use_roi_crop=use_roi_crop)
                        st.success(f"Salvo em: {saved['path'].name}")
                    except Exception as e:
                        st.error(f"Falha ao salvar OK: {e}")
            with top2:
                st.metric("Contador OK", usb_counts.get("OK", 0))

            r1, r2, r3 = st.columns(3)
            with r1:
                if st.button("⚠️ Desalinhado", key="usb_btn_desalinhado", use_container_width=True):
                    try:
                        saved = save_usb_manual_sample("NG_DESALINHADO", use_roi_crop=use_roi_crop)
                        st.warning(f"Salvo em: {saved['path'].name}")
                    except Exception as e:
                        st.error(f"Falha ao salvar: {e}")
                st.caption(f"Contador: {usb_counts.get('NG_DESALINHADO', 0)}")
            with r2:
                if st.button("🧩 Corpo Estranho", key="usb_btn_corpo", use_container_width=True):
                    try:
                        saved = save_usb_manual_sample("NG_CORPO_ESTRANHO", use_roi_crop=use_roi_crop)
                        st.warning(f"Salvo em: {saved['path'].name}")
                    except Exception as e:
                        st.error(f"Falha ao salvar: {e}")
                st.caption(f"Contador: {usb_counts.get('NG_CORPO_ESTRANHO', 0)}")
            with r3:
                if st.button("🔧 Danificado", key="usb_btn_danificado", use_container_width=True):
                    try:
                        saved = save_usb_manual_sample("NG_DANIFICADO", use_roi_crop=use_roi_crop)
                        st.warning(f"Salvo em: {saved['path'].name}")
                    except Exception as e:
                        st.error(f"Falha ao salvar: {e}")
                st.caption(f"Contador: {usb_counts.get('NG_DANIFICADO', 0)}")

            st.markdown("**Casos mistos (registrar na classe predominante por enquanto)**")
            m1, m2, m3 = st.columns(3)
            with m1:
                st.button("Desalinhado / Danificado", key="usb_mix_1", use_container_width=True, disabled=True)
            with m2:
                st.button("Desalinhado / Corpo Estranho", key="usb_mix_2", use_container_width=True, disabled=True)
            with m3:
                st.button("Danificado / Corpo Estranho", key="usb_mix_3", use_container_width=True, disabled=True)

            last_path = str(st.session_state.get("usb_last_saved_path", "")).strip()
            last_cls = str(st.session_state.get("usb_last_saved_class", "")).strip()
            if last_path:
                st.caption(f"Último salvamento: {last_cls} → {last_path}")
