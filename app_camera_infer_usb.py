...
def build_usb_ui_summary(res: Optional[dict]) -> dict:
    """Resumo visual para o classificador multiclasses USB."""
    res = res or {}
    ...
    probs = res.get("usb_probs") or res.get("probs") or {}
    probs = {str(k).strip().upper(): float(v) for k, v in dict(probs).items()}
    for name in ["OK", "NG_CORPO_ESTRANHO", "NG_DANIFICADO", "NG_DESALINHADO"]:
        probs.setdefault(name, 0.0)

    confidence = float(res.get("confidence", max(probs.values()) if probs else 0.0))
...
def render_resultado_usb(res: Optional[dict]) -> None:
    ui = build_usb_ui_summary(res)
    icon = "✔" if ui["status"] == "APROVADO" else "✖"
    extra = ""
    if ui.get("attention_flag"):
        extra = "<div style='margin-top:6px;font-size:12px;opacity:0.9;'>Aprovado com atenção / próximo do limite.</div>"

    html = textwrap.dedent(f"""    <div style="border-radius:12px;padding:16px;background:{ui['bg']};border:3px solid {ui['border']};text-align:center;margin-top:10px;">
      <div style="font-size:28px;font-weight:900;color:{ui['color']};margin-bottom:8px;">{icon} {ui['status']}</div>
      <div style="font-size:18px;font-weight:800;color:{ui['color']};">Classe detectada: {ui['class_name']}</div>
      <div style="margin-top:8px;font-size:14px;font-weight:700;color:{ui['color']};">Confiança: {float(ui['confidence']):.3f}</div>
      {extra}
    </div>
    """).strip()
...
def render_usb_probabilities(res: Optional[dict]) -> None:
    ui = build_usb_ui_summary(res)
    probs = ui["probs"]

    rows = []
    for name in ["OK", "NG_CORPO_ESTRANHO", "NG_DANIFICADO", "NG_DESALINHADO"]:
        val = float(probs.get(name, 0.0))
        rows.append(
            f'<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid #eef2f7;gap:12px;">'
            f'<div style="font-size:14px;font-weight:700;color:#1f2937;">{name}</div>'
            f'<div style="font-size:14px;font-weight:800;color:#0f172a;">{val:.3f}</div>'
            f'</div>'
        )

    rows_html = "".join(rows)

    html = textwrap.dedent(f"""    <div style="margin-top:10px;border:1px solid #d1d5db;border-radius:12px;background:#ffffff;padding:14px 16px;box-shadow:0 1px 2px rgba(0,0,0,0.04);">
      <div style="font-size:18px;font-weight:800;color:#111827;margin-bottom:6px;">Probabilidades</div>
      {rows_html}
    </div>
    """).strip()
...
USB_MULTICLASS_MODEL_PATH = BASE_DIR / "models" / "model_final.keras"
USB_MULTICLASS_LABELS_PATH = BASE_DIR / "models" / "labels.json"
USB_MULTICLASS_FALLBACK_DIRS = [
    BASE_DIR / "models",
    BASE_DIR / "models" / "usb_mobilenetv2",
    BASE_DIR / "outputs_usb_mobilenetv2_finetune",
    BASE_DIR / "outputs_usb_mobilenetv2",
]
...
def init_session():
    ...
    st.session_state.setdefault("cnt_ng_corpo_estranho", 0)
    st.session_state.setdefault("cnt_ng_danificado", 0)
    st.session_state.setdefault("cnt_ng_desalinhado", 0)
    ...
    st.session_state.setdefault("usb_model", None)
    st.session_state.setdefault("usb_labels", None)
    st.session_state.setdefault("usb_model_path", "")
    st.session_state.setdefault("usb_labels_path", "")
    st.session_state.setdefault("usb_model_img_size", (224, 224))
    ...
    st.session_state.setdefault("system_name", "SVC USB - Inspeção de Conectores USB")
    ...
    st.session_state.setdefault("email_sender_name", "SVC USB")
    ...
    st.session_state.setdefault("inspection_mode_usb", "CAMERA")
    ...
    # coleta manual USB
    st.session_state.setdefault("usb_use_roi_crop", False)
    st.session_state.setdefault("usb_last_saved_path", "")
    st.session_state.setdefault("usb_last_saved_class", "")
    st.session_state.setdefault("usb_roi_x0", USB_ROI_DEFAULT["x0"])
    st.session_state.setdefault("usb_roi_x1", USB_ROI_DEFAULT["x1"])
    st.session_state.setdefault("usb_roi_y0", USB_ROI_DEFAULT["y0"])
    st.session_state.setdefault("usb_roi_y1", USB_ROI_DEFAULT["y1"])
...
def apply_email_config_to_session(cfg: dict) -> None:
    ...
    st.session_state["email_subject_prefix"] = str(cfg.get("email_subject_prefix", "[SVC USB] Relatório de Auditoria"))
    st.session_state["email_sender_name"] = str(cfg.get("email_sender_name", "SVC USB"))
...
def collect_email_config_from_session() -> dict:
    return {
        ...
        "email_subject_prefix": str(st.session_state.get("email_subject_prefix", "[SVC USB] Relatório de Auditoria")).strip(),
        "email_sender_name": str(st.session_state.get("email_sender_name", "SVC USB")).strip(),
...
# ==========================================================
# DATASET USB — COLETA MANUAL / AUDITORIA
# ==========================================================
DATASET_USB_ROOT = BASE_DIR / "dataset_usb"
USB_CLASS_DIRS = {
    "OK": DATASET_USB_ROOT / "OK",
    "NG_CORPO_ESTRANHO": DATASET_USB_ROOT / "NG_CORPO_ESTRANHO",
    "NG_DANIFICADO": DATASET_USB_ROOT / "NG_DANIFICADO",
    "NG_DESALINHADO": DATASET_USB_ROOT / "NG_DESALINHADO",
}
for _p in USB_CLASS_DIRS.values():
    _p.mkdir(parents=True, exist_ok=True)

USB_ROI_DEFAULT = {"x0": 10, "x1": 90, "y0": 24, "y1": 82}
...
def map_defect_label(defect_type: str) -> str:
    ...
    if defect_type in {"NG_CORPO_ESTRANHO", "NG_DANIFICADO", "NG_DESALINHADO"}:
        return defect_type
    return "OK"
...
def build_defect_detail_code(res: dict | None) -> str:
    res = res or {}
    usb_class = str(res.get("usb_pred_class") or res.get("defect_type") or "OK").strip().upper()
    if usb_class in {"OK", "NG_CORPO_ESTRANHO", "NG_DANIFICADO", "NG_DESALINHADO"}:
        return usb_class
    return "OK"
...
def get_audit_counts_from_session() -> dict:
    return {
        "ok": int(st.session_state.get("cnt_ok", 0)),
        "ng_total": int(st.session_state.get("cnt_ng", 0)),
        "ng_corpo_estranho": int(st.session_state.get("cnt_ng_corpo_estranho", 0)),
        "ng_danificado": int(st.session_state.get("cnt_ng_danificado", 0)),
        "ng_desalinhado": int(st.session_state.get("cnt_ng_desalinhado", 0)),
    }
...
def get_usb_manual_counts() -> dict:
    counts = {}
    for class_name, folder in USB_CLASS_DIRS.items():
        try:
            counts[class_name] = count_jpgs(folder)
        except Exception:
            counts[class_name] = 0
    return counts


def crop_usb_roi(frame_bgr: np.ndarray) -> np.ndarray:
    if frame_bgr is None or frame_bgr.size == 0:
        return frame_bgr
    return crop_roi_percent(
        frame_bgr,
        int(st.session_state.get("usb_roi_x0", USB_ROI_DEFAULT["x0"])),
        int(st.session_state.get("usb_roi_x1", USB_ROI_DEFAULT["x1"])),
        int(st.session_state.get("usb_roi_y0", USB_ROI_DEFAULT["y0"])),
        int(st.session_state.get("usb_roi_y1", USB_ROI_DEFAULT["y1"])),
    )


def draw_usb_roi_overlay(frame_bgr: np.ndarray) -> np.ndarray:
    if frame_bgr is None or frame_bgr.size == 0:
        return frame_bgr
    img = frame_bgr.copy()
    h, w = img.shape[:2]
    x0 = int(clamp01(float(st.session_state.get("usb_roi_x0", USB_ROI_DEFAULT["x0"])) / 100.0) * w)
    x1 = int(clamp01(float(st.session_state.get("usb_roi_x1", USB_ROI_DEFAULT["x1"])) / 100.0) * w)
    y0 = int(clamp01(float(st.session_state.get("usb_roi_y0", USB_ROI_DEFAULT["y0"])) / 100.0) * h)
    y1 = int(clamp01(float(st.session_state.get("usb_roi_y1", USB_ROI_DEFAULT["y1"])) / 100.0) * h)
    x0, x1 = sorted([x0, x1])
    y0, y1 = sorted([y0, y1])
    cv2.rectangle(img, (x0, y0), (x1, y1), (0, 0, 255), 2)
    cv2.putText(img, "ROI USB", (x0, max(18, y0 - 8)), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 0, 255), 2, cv2.LINE_AA)
    return img


def save_usb_manual_sample(class_name: str, use_roi_crop: bool | None = None, jpeg_quality: int = 92) -> dict:
    class_name = str(class_name or "").strip().upper()
    if class_name not in USB_CLASS_DIRS:
        raise ValueError(f"Classe USB inválida: {class_name}")

    src = capture_source_frame_for_learning()
    if src is None:
        raise RuntimeError("Sem frame para salvar (ligue a câmera ou tenha um last_frame).")

    if use_roi_crop is None:
        use_roi_crop = bool(st.session_state.get("usb_use_roi_crop", False))

    img_to_save = crop_usb_roi(src) if bool(use_roi_crop) else src.copy()

    stamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    serial_number = normalize_serial_qr(st.session_state.get("serial_qr_code", ""))
    serial_tag = sanitize_filename(serial_number) if serial_number else "SEM_SERIAL"
    suffix = "roi" if bool(use_roi_crop) else "raw"
    file_name = f"{stamp}__{class_name}__{serial_tag}__{suffix}.jpg"
    path = USB_CLASS_DIRS[class_name] / file_name

    save_jpg(path, img_to_save, quality=jpeg_quality)

    st.session_state["usb_last_saved_path"] = str(path)
    st.session_state["usb_last_saved_class"] = class_name
    st.session_state["usb_use_roi_crop"] = bool(use_roi_crop)

    return {"path": path, "class_name": class_name, "use_roi_crop": bool(use_roi_crop)}
...
def _resolve_usb_multiclass_paths() -> tuple[Path, Path, tuple[int, int]]:
    candidates: list[tuple[Path, Path]] = []

    direct_label_candidates = [
        USB_MULTICLASS_LABELS_PATH,
        BASE_DIR / "models" / "labels",
        BASE_DIR / "models" / "labels_usb.json",
    ]
    if USB_MULTICLASS_MODEL_PATH.exists():
        for lp in direct_label_candidates:
            if lp.exists():
                candidates.append((USB_MULTICLASS_MODEL_PATH, lp))

    for folder in USB_MULTICLASS_FALLBACK_DIRS:
        model_candidates = [folder / "model_final.keras", folder / "best_finetuned.keras", folder / "best_model.keras", folder / "model_usb.keras"]
        labels_candidates = [folder / "labels.json", folder / "labels", folder / "labels_usb.json"]
        for model_candidate in model_candidates:
            if not model_candidate.exists():
                continue
            for labels_candidate in labels_candidates:
                if labels_candidate.exists():
                    candidates.append((model_candidate, labels_candidate))
                    break
    ...
    for model_path, labels_path in candidates:
        ...
        if key not in seen:
            seen.add(key)
            unique_candidates.append((Path(model_path), Path(labels_path)))

    for model_path, labels_path in unique_candidates:
        try:
            labels = json.loads(Path(labels_path).read_text(encoding="utf-8"))
            if isinstance(labels, list) and len(labels) >= 2:
                return Path(model_path), Path(labels_path), (224, 224)
        except Exception:
            continue

    raise FileNotFoundError(
        "Modelo USB multiclasses não encontrado. Verifique models/model_final.keras + labels(.json) ou outputs_usb_mobilenetv2_finetune/."
    )


def ensure_usb_model_loaded_or_raise(blocking: bool = False):
    if st.session_state.get("usb_model") is not None and st.session_state.get("usb_labels") is not None:
        return
    ...
    try:
        model_path, labels_path, img_size = _resolve_usb_multiclass_paths()
        st.session_state["usb_labels"] = json.loads(Path(labels_path).read_text(encoding="utf-8"))
        st.session_state["usb_model"] = tf.keras.models.load_model(model_path)
        st.session_state["usb_model_path"] = str(model_path)
        st.session_state["usb_labels_path"] = str(labels_path)
        st.session_state["usb_model_img_size"] = tuple(img_size)
    finally:
...
def ensure_active_model_loaded_or_raise(blocking: bool = False):
    """Garante que o modelo USB multiclasses esteja carregado."""
    ensure_usb_model_loaded_or_raise(blocking=blocking)
    if st.session_state.get("usb_model") is None or st.session_state.get("usb_labels") is None:
        raise RuntimeError("Modelo USB multiclasses não carregado.")
...
def run_infer_dual_on_uploaded_frame(frame_bgr: np.ndarray, file_name: str = "upload", update_metrics: bool = False):
    """Executa a inspeção USB multiclasses em uma imagem enviada manualmente (modo laboratório)."""
    try:
    ...
    try:
        ...
        st.session_state["last_warning"] = f"Modo Laboratório: inspeção por upload executada: {file_name}"
        return res
    except Exception as e:
...
def update_metrics_and_history(res: dict) -> None:
    ...
    classe = str(res.get("classe", "OK")).strip().upper()

    if classe == "OK":
        st.session_state["cnt_ok"] = int(st.session_state.get("cnt_ok", 0)) + 1

    elif classe == "NG_CORPO_ESTRANHO":
        st.session_state["cnt_ng"] = int(st.session_state.get("cnt_ng", 0)) + 1
        st.session_state["cnt_ng_corpo_estranho"] = int(
        st.session_state.get("cnt_ng_corpo_estranho", 0)
    ) + 1

    elif classe == "NG_DANIFICADO":
        st.session_state["cnt_ng"] = int(st.session_state.get("cnt_ng", 0)) + 1
        st.session_state["cnt_ng_danificado"] = int(
        st.session_state.get("cnt_ng_danificado", 0)
    ) + 1

    elif classe == "NG_DESALINHADO":
        st.session_state["cnt_ng"] = int(st.session_state.get("cnt_ng", 0)) + 1
        st.session_state["cnt_ng_desalinhado"] = int(
        st.session_state.get("cnt_ng_desalinhado", 0)
    ) + 1
...
def append_log_csv(row: dict):
    ...
    log_path = LOG_DIR / f"inspecao_usb_{today}.csv"
    ...
    # reescreve preservando os dados compatíveis.
    if file_exists:
        try:
...
def get_cs_code(res: dict, th: float = 0.0) -> tuple[str, str]:
    """Código curto para UI/CSV no SVC USB.
    ...
    if defect_type in ("OK", "NG_CORPO_ESTRANHO", "NG_DANIFICADO", "NG_DESALINHADO"):
        conf = float(res.get("confidence", 0.0))
        detail = f"class={defect_type} | conf={conf:.3f}"
    ...
    if pred_class in ("OK", "NG_CORPO_ESTRANHO", "NG_DANIFICADO", "NG_DESALINHADO"):
        return pred_class, f"class={pred_class} | conf={conf:.3f}"

    return "OK", f"class=OK | conf={conf:.3f}"
...
def _save_report_charts(snapshot: dict, stamp: str) -> list[Path]:
    ...
    ok = int(snapshot["ok"])
    ng = int(snapshot["ng"])
    yield_pct = float(snapshot["yield_pct"])

    # =========================
    # Chart 1: Tendência de Yield (%)
    # =========================
    # No relatório estático, usamos um resumo visual:
    # total produzido, falhas e yield final do período.
    ...
    plt.title(f"Tendência de Yield (%) - Yield final: {yield_pct:.2f}%")
    plt.grid(True, axis="y", alpha=0.3)

    p1 = REPORTS_DIR / f"audit_report_{stamp}_yield.png"
    fig1.savefig(p1, bbox_inches="tight", dpi=180)
    plt.close(fig1)
    charts.append(p1)

    # =========================
    # Chart 2: Tipos de falha (USB)
    # =========================
    labels = [
        "NG_CORPO_ESTRANHO",
        "NG_DANIFICADO",
    ...
    values = [
        int(audit.get("ng_corpo_estranho", 0)),
        int(audit.get("ng_danificado", 0)),
        int(audit.get("ng_desalinhado", 0))
    ]

    fig2 = plt.figure(figsize=(8, 4))
    plt.bar(labels, values)
    plt.ylabel("Ocorrências")
    plt.title("Detalhamento das Falhas - USB")
    plt.xticks(rotation=15)
    plt.grid(True, axis="y", alpha=0.3)

    p2 = REPORTS_DIR / f"audit_report_{stamp}_falhas.png"
    fig2.savefig(p2, bbox_inches="tight", dpi=180)
    plt.close(fig2)
    charts.append(p2)
...
def _save_report_charts(snapshot: dict, stamp: str) -> list[Path]:
    ...
    values = [
        audit["ng_corpo_estranho"],
        audit["ng_danificado"],
        audit["ng_desalinhado"]
    ]

    fig = plt.figure(figsize=(8, 4))
    plt.bar(labels, values)
    plt.ylabel("Ocorrências")
    plt.title("Detalhamento de Falhas - USB")
    plt.xticks(rotation=15)
...
def _build_report_html(snapshot: dict, charts: list[Path] | None = None) -> str:
    audit = snapshot["audit"]
    charts = charts or []

    def _img_html(path: Path, title: str) -> str:
        try:
            data = base64.b64encode(Path(path).read_bytes()).decode("ascii")
            return (
                f'<div style="margin:18px 0;">'
                f'<div style="font-weight:700;margin-bottom:6px;">{title}</div>'
                f'<img src="data:image/png;base64,{data}" style="max-width:100%;border:1px solid #d1d5db;border-radius:8px;" />'
                f'</div>'
            )
        except Exception:
            return ""
    ...
    charts_html = ""
    if len(charts) > 0:
        charts_html += _img_html(charts[0], "Gráfico 1 – Produção e Yield")
    if len(charts) > 1:
        charts_html += _img_html(charts[1], "Gráfico 2 – Detalhamento das Falhas")

    return f"""<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="utf-8">
    <title>Resumo Relatório de Auditoria - SVC USB</title>
</head>
<body style="font-family:Arial,sans-serif;font-size:14px;color:#111827;max-width:980px;margin:0 auto;padding:16px;">
    <h2 style="margin-bottom:8px;">Resumo do Relatório de Auditoria – SVC USB</h2>

    <p>
        <b>Emitido em:</b> {snapshot['generated_at']}<br>
        <b>Período:</b> {snapshot['period_start']} até {snapshot['generated_at']}<br>
        <b>Linha:</b> {snapshot['line_name']} &nbsp; | &nbsp;
        <b>Equipamento:</b> {snapshot['equipment_id']}<br>
        <b>Modelo:</b> {snapshot['model_name']} &nbsp; | &nbsp;
        <b>OP:</b> {snapshot['production_order']}<br>
        <b>Último Inspection ID:</b> {snapshot['inspection_id']}
    </p>
    ...
    <table style="border-collapse:collapse;margin:8px 0;" border="1" cellpadding="6">
        ...
        <tr>
            ...
            <td>{snapshot['ok']}</td>
            <td>{snapshot['ng']}</td>
            <td>{snapshot['yield_pct']:.2f}%</td>
        </tr>
    </table>

    <p><b>Detalhamento das classes NG:</b><br>
       NG_CORPO_ESTRANHO: {audit['ng_corpo_estranho']}<br>
       NG_DANIFICADO: {audit['ng_danificado']}<br>
       NG_DESALINHADO: {audit['ng_desalinhado']}</p>
...
def generate_audit_report_files() -> tuple[Path, Path]:
    ...
    pdf_path = REPORTS_DIR / f"audit_report_usb_{stamp}.pdf"
    html_path = REPORTS_DIR / f"audit_report_usb_{stamp}.html"
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        line("SVC USB – Computer Vision System for USB Inspection", size=18, weight="bold", step=0.035)
        ...
        line("Detalhamento das classes NG", size=12, weight="bold", step=0.03)
        line(f"NG_CORPO_ESTRANHO: {audit['ng_corpo_estranho']}", size=10)
        line(f"NG_DANIFICADO: {audit['ng_danificado']}", size=10)
        line(f"NG_DESALINHADO: {audit['ng_desalinhado']}", size=10, step=0.034)

        line("Observações", size=12, weight="bold", step=0.03)
        line("Relatório gerado automaticamente pelo sistema SVC USB.", size=10)
        ...
        for idx, chart_path in enumerate(charts, start=1):
            try:
                ...
                axc = figc.add_axes([0.06, 0.08, 0.88, 0.84])
                axc.imshow(img)
                axc.axis("off")
                figc.suptitle(
                    "Gráfico de Produção e Yield" if idx == 1 else "Gráfico de Detalhamento das Falhas",
                    fontsize=16,
                    fontweight="bold",
                    y=0.96,
                )
    ...
    html_content = _build_report_html(snapshot, charts)
    html_path.write_text(html_content, encoding="utf-8")

    st.session_state["last_report_pdf"] = str(pdf_path)
    st.session_state["last_report_html"] = str(html_path)
    st.session_state["last_report_generated_at"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
def _build_email_subject() -> str:
    prefix = str(
        st.session_state.get("email_subject_prefix", "[SVC USB]")
    ).strip()
...
def send_report_email(pdf_path: str | Path, html_path: str | Path | None = None) -> tuple[bool, str]:
    ...
    smtp_user = str(st.session_state.get("smtp_user", "")).strip()
    smtp_password = str(st.session_state.get("smtp_password", "")).strip()
    sender_name = str(st.session_state.get("email_sender_name", "SVC USB")).strip() or "SVC Inspeção de USB"
...
# STREAMLIT APP
# ==========================================================
st.set_page_config(page_title="SVC USB — Inspeção de Conectores USB", layout="wide")
st.markdown("""
<div class="app-title-fixed">
    SVC USB — Inspeção de Conectores USB
</div>
""", unsafe_allow_html=True)
# ==========================================================
...
with st.sidebar:
    ...
    with st.expander("ℹ️ Sobre o Sistema", expanded=False):
        ...
        st.markdown("- **Versão:** v0.1.0")
        st.markdown("- **Status:** Research Prototype – Industrial ROI Calibration")
...
is_eng = (
    st.session_state.get("user_mode") == "ENG"
    and st.session_state.get("eng_unlocked", False)
)
...
st.sidebar.checkbox("Usar classificador USB multiclasses", value=True, key="use_mnv2_prod", disabled=True)
if is_eng:
...
if is_eng:
    ...
    st.sidebar.text_input(
        ...
        help="Nome exibido no e-mail. Ex.: SVC USB"
    )
    st.sidebar.text_input(
        "SMTP server",
...
btn_capture = st.sidebar.button("📸 Capturar + Inspecionar USB", type="primary", width='stretch')
...
if is_eng:
    st.sidebar.divider()
    st.sidebar.header("🧪 Modo Laboratório (Upload)")
    st.sidebar.caption("Use fotos do dataset ou imagens já capturadas para testar o SVC USB sem câmera e sem sensor.")

    inspection_mode_usb = st.sidebar.radio(
        "Fonte da inspeção",
        options=["CAMERA", "LABORATORIO"],
        index=0 if str(st.session_state.get("inspection_mode_usb", "CAMERA")).upper() == "CAMERA" else 1,
        format_func=lambda x: "Câmera" if x == "CAMERA" else "Laboratório (upload)",
        horizontal=False,
        help="No modo Laboratório, a imagem enviada por upload passa a ser a fonte principal da inspeção."
    )
    st.session_state["inspection_mode_usb"] = str(inspection_mode_usb).upper()

    uploaded_test_file = st.sidebar.file_uploader(
        "Enviar foto do produto",
        type=["jpg", "jpeg", "png", "bmp"],
        key="eng_upload_test_file",
        help="A imagem enviada será usada como entrada offline para a mesma lógica USB multiclasses do SVC."
    )
    ...
    with c_up1:
        if st.sidebar.button("🧪 Inspecionar imagem (Laboratório)", width='stretch'):
    ...
    if str(st.session_state.get("inspection_mode_usb", "CAMERA")).upper() == "LABORATORIO":
        if st.session_state.get("upload_test_frame") is None:
            st.sidebar.info("Modo Laboratório ativo. Envie uma imagem para testar offline.")
        else:
            st.sidebar.success("Modo Laboratório ativo: a inspeção usará a imagem enviada por upload.")
...
# ==========================================================
def infer_dual_on_frame(frame_bgr: np.ndarray):
    """Inferência USB multiclasses em ROI única.

    Mantém chaves legadas para não quebrar a UI e rotinas de salvamento já existentes.
    """
    if frame_bgr is None or getattr(frame_bgr, "size", 0) == 0:
        raise ValueError("Frame inválido para inferência USB.")

    ensure_usb_model_loaded_or_raise(blocking=True)

    model = st.session_state.get("usb_model")
    labels = st.session_state.get("usb_labels") or []
    img_size = tuple(st.session_state.get("usb_model_img_size", (224, 224)))

    if model is None or not labels:
        raise RuntimeError("Modelo USB multiclasses não carregado.")

    roi_usb = crop_usb_roi(frame_bgr)
    if bool(st.session_state.get("normalize_lab_equalize", False)):
        roi_usb = equalize_lab_bgr(roi_usb)

    rgb = cv2.cvtColor(roi_usb, cv2.COLOR_BGR2RGB)
    x = tf.image.resize(rgb, img_size, method="bilinear")
    x = tf.cast(x, tf.float32)
    x = tf.expand_dims(x, axis=0)

    probs_arr = model.predict(x, verbose=0)[0]
    probs_arr = np.asarray(probs_arr, dtype=np.float32)
    probs = {str(labels[i]).strip().upper(): float(probs_arr[i]) for i in range(len(labels))}

    pred_idx = int(np.argmax(probs_arr))
    pred_class = str(labels[pred_idx]).strip().upper()
    confidence = float(probs_arr[pred_idx])

    for name in ["OK", "NG_CORPO_ESTRANHO", "NG_DANIFICADO", "NG_DESALINHADO"]:
        probs.setdefault(name, 0.0)

    aprovado = pred_class == "OK"
    ng_score = float(max(probs.get("NG_CORPO_ESTRANHO", 0.0), probs.get("NG_DANIFICADO", 0.0), probs.get("NG_DESALINHADO", 0.0)))

    return {
        "usb_mode": True,
        "roi_esq": roi_usb,
        "roi_dir": roi_usb.copy(),
        "roi_usb": roi_usb,
        "usb_pred_class": pred_class,
        ...
        "cls_esq": pred_class,
        "cls_dir": pred_class,
        "p_pres_esq": float(probs.get("OK", 0.0)),
        "p_pres_dir": float(probs.get("OK", 0.0)),
        "conf_esq": confidence,
        "conf_dir": confidence,
        "cls_mnv2_esq": pred_class,
        "cls_mnv2_dir": pred_class,
        "prob_ng_esq": ng_score,
        "prob_ng_dir": ng_score,
        "prob_ok_esq": float(probs.get("OK", 0.0)),
        "prob_ok_dir": float(probs.get("OK", 0.0)),
        "thr_ng": float(st.session_state.get("confidence_threshold", 0.50)),
        "thr_ng_ok": float(st.session_state.get("confidence_threshold", 0.50)),
        "thr_ng_ng": float(st.session_state.get("confidence_threshold", 0.50)),
        "margin_esq": confidence,
        "margin_dir": confidence,
        "decision_band_esq": pred_class,
        "decision_band_dir": pred_class,
        "attention_esq": False,
        "attention_dir": False,
        "attention_flag": False,
        "probs_esq": probs,
        "probs_dir": probs,
        "defect_esq": pred_class,
        "defect_dir": pred_class,
...
def run_capture_infer_dual(trigger_source: str = "button"):
    ...
    try:
        ...
        ensure_active_model_loaded_or_raise(blocking=True)
        if st.session_state.get("usb_model") is None or st.session_state.get("usb_labels") is None:
            raise RuntimeError("Modelo USB multiclasses não carregado (após ensure).")
    except Exception as e:
        st.session_state["last_error"] = f"Falha ao carregar modelo atual: {e}"
        st.session_state["last_result"] = None
        return
    ...
    try:
        ...
        system_name = str(st.session_state.get("system_name", "SVC USB Inspection System"))
...
# Modo laboratório tem prioridade quando houver imagem carregada.
elif str(st.session_state.get("inspection_mode_usb", "CAMERA")).upper() == "LABORATORIO" and st.session_state.get("upload_test_frame") is not None:
    frame = st.session_state.get("upload_test_frame").copy()
...
with colA:
    with st.container(border=True):
        ...
        if frame is None:
            st.warning("Sem frame. Ligue a câmera ou use o Modo Laboratório por upload.")
        else:
            upload_name = str(st.session_state.get("upload_test_name", "")).strip()
            inspection_mode_now = str(st.session_state.get("inspection_mode_usb", "CAMERA")).upper()
            if upload_name and (inspection_mode_now == "LABORATORIO" or not st.session_state.get("camera_on", False)):
                st.caption(f"Modo Laboratório ativo · imagem carregada: {upload_name}")
...
with colB:
    try:
        ...
        if frame is not None:
            try:
                roi_preview = crop_usb_roi(frame)
            except Exception:
                roi_preview = None

        with st.container(border=True):
            st.markdown("#### ROI do USB")
            st.caption("Recorte usado na inferência / auditoria do conector USB.")

            if roi_preview is not None:
                st.image(cv2.cvtColor(roi_preview, cv2.COLOR_BGR2RGB), width=320)
            else:
                st.info("Sem ROI disponível no momento.")

        if res is not None:
            render_resultado_usb(res)
########    render_usb_probabilities(res)
        else:
            st.info("Ainda não foi feita uma inspeção.")

    except Exception as e:
        st.error(f"Erro ao renderizar painel direito USB: {e}")
        if show_debug:
            st.exception(e)

with colB:
    if is_eng:
        with st.expander("📋 Resumo de Coleta USB", expanded=True):
            usb_counts = get_usb_manual_counts()
            c1, c2, c3, c4 = st.columns(4)

            with c1:
                st.metric("OK", usb_counts.get("OK", 0))
            with c2:
                st.metric("NG_CORPO_ESTRANHO", usb_counts.get("NG_CORPO_ESTRANHO", 0))
            with c3:
                st.metric("NG_DANIFICADO", usb_counts.get("NG_DANIFICADO", 0))
            with c4:
                st.metric("NG_DESALINHADO", usb_counts.get("NG_DESALINHADO", 0))

    with st.expander("🧾 Painel detalhado de auditoria", expanded=True):
        a1, a2, a3 = st.columns(3)
        with a1:
            st.metric("NG_CORPO_ESTRANHO", int(st.session_state.get("cnt_ng_corpo_estranho", 0)))
        with a2:
            st.metric("NG_DANIFICADO", int(st.session_state.get("cnt_ng_danificado", 0)))
        with a3:
            st.metric("NG_DESALINHADO", int(st.session_state.get("cnt_ng_desalinhado", 0)))

        b1, b2, b3 = st.columns(3)
        with b1:
            st.metric("OK", int(st.session_state.get("cnt_ok", 0)))
        with b2:
            st.metric("OK com atenção", int(st.session_state.get("cnt_ok_attention", 0)))
        with b3:
    ...
    with btn_col:
        if st.button("📄 Gerar Relatório de Auditoria", key="btn_generate_audit_report_usb", use_container_width=True):
            try:
                pdf_path, html_path = generate_audit_report_files()
